...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // grab *all* filter controls, no matter which class you used
  const filters = Array.from(
    document.querySelectorAll(
      '.controls__filter select, .controls__filter input[type="date"],' +
      '.filter-dropdown select, .filter-dropdown input[type="date"]'
    )
  );

  // 1) blur any open <select> when we click the gear
  toggleArea.addEventListener('mousedown', () => {
  ...
  function setFiltersDisabled(disabled) {
    filters.forEach(f => f.disabled = disabled);
  }

  // 2) open/close modal & disable filters
  toggleArea.addEventListener('click', () => {
    const opening = !profileModal.classList.contains('show');
    profileModal.classList.toggle('show', opening);
    document.body.classList.toggle('modal-open', opening);
    setFiltersDisabled(opening);
  });

  // 3) clicking outside the modal restores filters
